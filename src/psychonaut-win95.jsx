  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:2100, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"stretch", justifyContent:"center" }}>
      <div style={{ width:"100%", maxWidth:480, background:"var(--surface)", boxShadow:"var(--raised)", margin:"0 auto", display:"flex", flexDirection:"column",
        ...
        <div style={{ flex:1, overflowY:"auto", overscrollBehavior:"contain", padding:"4px 16px calc(20px + max(env(safe-area-inset-bottom,0px), var(--sab,0px)))" }}>
          <P>Это приложение помогает не растерять психоделический опыт, а прожить его в конкретных изменениях. Ты готовишься до, записываешь по горячим следам и спокойно разбираешь после. Ниже коротко о каждой части и о том, как они связаны.</P>

          <H>Заметки и сессии</H>
          <P>Сердце приложения. Каждый опыт это отдельная сессия, которую ты заполняешь по шагам: настройка и намерение до, запись сразу после, сложный опыт, оценки по граням жизни. Незавершённую сессию можно продолжить с любого шага. Завершённую изменить уже нельзя, но к ней можно дописывать интеграцию со временем.</P>

          <H>Шаги внутри сессии</H>
          <P>Настройка: вещество, доза, место, кто рядом, состояние и страхи до опыта. Намерение: зачем ты идёшь, что хочешь отпустить и увидеть. Сразу после: самое яркое, ощущения в теле, как откликнулось намерение, что осталось незавершённым. Сложный опыт: отдельный бережный шаг о тяжёлом, если оно было. Грани: как ты оцениваешь разные области жизни после опыта.</P>

          <H>Черновики и привязанные мысли</H>
          <P>Быстрые заметки, которые не привязаны к конкретной сессии. Любой черновик можно привязать к сессии, и тогда он войдёт в её разбор как часть материала. Удобно, когда мысль пришла между сессиями.</P>

          <H>Зарисовки <Pay /></H>
          <P>Рисование пальцем прямо в приложении, чтобы выразить то, что трудно сказать словами. Рисование и сохранение рисунка доступны в полной версии.</P>

          <H>Разбор от Claude <Pay /></H>
          <P>Спокойное чтение твоей сессии: паттерны, смысл, связь намерения и того, что пришло, бережная работа с трудным и шаги в жизнь. Разбор приходит отдельным файлом в чат бота. Доступен в полной версии.</P>

          <H>Трекер граней</H>
          <P>Показывает, как меняются твои оценки областей жизни от сессии к сессии. Твоя собственная линия оценок бесплатна. Вторая линия, оценка от Claude по тексту сессии, в полной версии.</P>

          <H>База знаний</H>
          <P>Статьи о подготовке, о самом опыте и об интеграции. Читается свободно, без ограничений.</P>

          <H>Кризис</H>
          <P>Что делать в трудный момент: упражнения на заземление, простые шаги, куда обратиться за живой помощью. Открыто всегда и бесплатно.</P>

          <H>Музыка и Змейка</H>
          <P>Музыка для настройки и спокойствия. Змейка это маленькая игра для передышки <Pay />. Змейка доступна в полной версии.</P>

          <H>Что с чем связано</H>
          <P>Разбор строится из всей сессии, привязанных к ней мыслей и предыдущих разборов этой же сессии. Поэтому если удалить сессию, вместе с ней исчезнут и её разборы. Трекер собирается из оценок граней в сессиях: удалишь сессию, и из трекера уйдёт её след. Рисунок сохраняется только на твой телефон, в приложении он не остаётся. Завершённую сессию нельзя переписать, можно только дописывать интеграцию.</P>
          ...
          <H>Полная версия</H>
          <P>По годовой подписке открываются: разбор от Claude, оценка граней от Claude в трекере, зарисовки с сохранением и игра «Змейка». Всё остальное доступно бесплатно.</P>
        </div>
      </div>
    </div>
  );
      ...
      {about && (
        <div onClick={() => setAbout(false)} style={{ position:"fixed", inset:0, zIndex:2000,
          ...
          <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:340, background:"var(--surface)", boxShadow:"var(--raised)" }}>
            ...
            <div style={{ padding:"12px 14px 16px", fontSize:13, color:"#000", lineHeight:1.7, fontFamily:"'Montserrat', sans-serif" }}>
              Заметки психонавта это инструмент интеграции психоделического опыта. Место, где опыт не теряется, а превращается в проживаемые изменения: подготовка до, запись по горячим следам, спокойный разбор после.
              <div style={{ display:"flex", gap:8, marginTop:16 }}>
                <button onClick={() => { setAbout(false); setManualOpen(true); }}
                  style={{ flex:1, height:34, WebkitAppearance:"none", appearance:"none", borderRadius:0, background:"#c0c0c0", border:"none", cursor:"pointer", fontSize:13, fontWeight:700, color:"#000", boxShadow:"inset -1px -1px #000, inset 1px 1px #fff, inset -2px -2px #808080, inset 2px 2px #dfdfdf" }}>Как пользоваться</button>
                <button onClick={() => setAbout(false)}
                  style={{ flex:1, height:34, WebkitAppearance:"none", appearance:"none", borderRadius:0, background:"#c0c0c0", border:"none", cursor:"pointer", fontSize:13, fontWeight:700, color:"#000", boxShadow:"inset -1px -1px #000, inset 1px 1px #fff, inset -2px -2px #808080, inset 2px 2px #dfdfdf" }}>OK</button>
              </div>
            </div>
          </div>
        </div>
