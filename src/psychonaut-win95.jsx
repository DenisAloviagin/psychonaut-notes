    ...
    <div style={{ position:"fixed", inset:0, zIndex:2100, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"stretch", justifyContent:"center" }}>
      <div style={{ width:"100%", maxWidth:480, background:"var(--surface)", boxShadow:"var(--raised)", margin:"0 auto", display:"flex", flexDirection:"column",
        ...
        <div style={{ flex:1, overflowY:"auto", overscrollBehavior:"contain", padding:"12px 16px calc(20px + max(env(safe-area-inset-bottom,0px), var(--sab,0px)))" }}>
          ...
          <P>Когда почувствуешь, что готов, попроси разбор. Он прочитает всю твою сессию целиком и мягко покажет то, что трудно увидеть изнутри: повторяющиеся темы, смысл опыта, связь намерения и того, что пришло, и куда можно сделать первый шаг. Разбор приходит отдельным файлом в чат бота, чтобы можно было спокойно перечитать. Доступен в полной версии.</P>
          ...
          <H>Музыка</H>
          <P>Твой личный список музыки для настройки и тишины. Ты добавляешь ссылки на треки, которые тебя заземляют или открывают, и они сохраняются под рукой. Воспроизведение открывается во внешнем приложении по ссылке, не внутри Telegram. Бесплатно.</P>
