...
const css = `
  ...
  * {
    ...
    font-family: "Pixelated MS Sans Serif","MS Sans Serif",Tahoma,Geneva,sans-serif !important;
...
function AccentBar() {
  ...
  const actions = {
    "_": () => { try { window.scrollTo({ top: 0, behavior: "smooth" }); } catch (e) {} },
...
function NavBar({ active, onChange, onJournalTab, onPrivacy }) {
...
function buildPrompt(session) {
  ...
  let text = `Ты — инструмент психоделической интеграции. Твоя роль: быть зеркалом, не терапевтом. Отражать паттерны, задавать вопросы для углубления, не давать советов и не ставить диагнозов.

Тон: живой, уважительный, без клише. Не используй слова «трансформация», «исцеление», «путешествие» как клише. Говори конкретно о том что написал человек.

Структура ответа:
1. **Что я вижу** — 2-3 абзаца: паттерны, противоречия, что повторяется, что бросается в глаза. Только то что есть в тексте — никаких домыслов.
2. **Намерение и реальность** — как то что было написано как намерение соотносится с тем что пришло.
3. **Вопросы для углубления** — 3 конкретных вопроса основанных на том что написал человек. Не общие — именно из его текста.

Не заканчивай мотивационными фразами. Не говори что всё хорошо.
...
Что осталось незавершённым: ${session.after_open}`;
  if (session.after_extra) text += `
Дополнительные заметки: ${session.after_extra}`;

  // Step 4: Difficult experience
  if (session.diff_what || session.diff_feel || session.diff_value || session.diff_helps) {
    text += `
...
// ── Journal list ──────────────────────────────────────────────────────────────
function JournalList({ sessions, isPremium, onNew, onOpen, onUpgrade, onPrivacy }) {
  return (
    <Screen>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:4, padding:"4px 0" }}>
          {sessions.map(s => (
            <button key={s.id} onClick={() => onOpen(s)}
              ...
              <div style={{ fontSize:9, color:T.muted, textAlign:"center" }}>{s.date}</div>
            </button>
          ))}
        </div>
      )}
...
Психологическая подготовка. Не принимать в периоды острого стресса, депрессии или жизненных трудностей — грибы усиливают то что уже есть внутри. Людям с историей психоза, биполярного расстройства — только в клиническом контексте.
...
Важно: откровения психоделического опыта часто нельзя воспринимать буквально. Требуется процесс расшифровки — и он гораздо плодотворнее с подготовленным человеком.
...
Вместо того чтобы встретиться с болью, тревогой, непроработанной травмой — человек прячется за инсайтом. «Я всё понял на церемонии», «я уже через это прошёл» — и реальная работа не делается. Это выглядит как интеграция. Но им не является.
...
Погоня за кайфом почти никогда не срабатывает. Это как снимать новую фотографию поверх другой на том же кадре плёнки — изображение будет замутнено.
...
Снижение рисков. Главное — сет и сеттинг. Не принимать в периоды стресса и депрессии. При сборе в природе — обязательна точная идентификация: некоторые виды рода Galerina смертельно ядовиты. Людям с историей психоза, биполярного расстройства — только в клиническом контексте.
...
Снижение рисков. Концентрация алкалоидов крайне вариабельна — дозировка практически непредсказуема. Критически важна точная идентификация: Amanita phalloides (бледная поганка) внешне похожа и смертельно ядовита. Amanita pantherina — более тяжёлая интоксикация. Не рекомендуется для неопытных пользователей.
...
Это выглядит как интеграция. Но им не является.
...
export default function App() {
  ...
  const saveFlowStep = (step) => {
    ...
    // Update _step in draft
    if (draftId) {
      setSessions(prev => prev.map(s =>
        s.id === draftId ? { ...s, _step: step } : s
      ));
    }
  };
  ...
  function startNew() {
    ...
    if (existingDraft) {
      const step = existingDraft._step || 0;
      setFlowData(existingDraft);
      setFlowStep(step);
...
Где N — целое число от 1 до 10. Если по какой-то области записей нет — поставь 5.`;
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", minHeight:"100vh", position:"relative", overflowX:"hidden" }}>
      ...
      <NavBar active={tab} onPrivacy={() => { setTab("journal"); setJournalView("privacy"); }} onChange={id => {
        setTab(id);
        if (id === "journal") {
          try {
